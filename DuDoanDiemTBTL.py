...
try:
    ...
    qt = pd.read_csv("./data/qt-public.csv")    # cột: hash, diemqt
    th = pd.read_csv("./data/th-public.csv")    # cột: hash, TH
    ck = pd.read_csv("./data/ck-public.csv")    # cột: hash, CK

    # Đổi tên cột cho đồng bộ
    qt = qt.rename(columns={'diemqt': 'QT'})

    # Gộp theo hash
    df_gt = qt.merge(th, on='hash', how='inner')\
        .merge(ck, on='hash', how='inner')

    # Ép kiểu QT, TH, CK về số
    df_gt['QT'] = pd.to_numeric(df_gt['QT'], errors='coerce')
    df_gt['TH'] = pd.to_numeric(df_gt['TH'], errors='coerce')
    df_gt['CK'] = pd.to_numeric(df_gt['CK'], errors='coerce')

    # Tính điểm trung bình tích lũy
    df_gt['TBTL'] = 0.2 * df_gt['QT'] + 0.4 * df_gt['TH'] + 0.4 * df_gt['CK']


    # ✅ Giờ df_gt có: hash, QT, TH, CK, TBTL
...
try:
    ...
    if 'TBTL' not in df_gt.columns:
        print("❌ Không tìm thấy cột 'TBTL' trong file ground truth")
        print(f"Các cột có sẵn: {df_gt.columns.tolist()}")
        exit(1)
    ...
    feature_columns = [col for col in student_features.columns 
                      if col not in ["username", "hash", "TBTL", "first_submission", "last_submission", "QT", "TH", "CK"]]
    
    X_train = train_data[feature_columns]
    y_train = pd.to_numeric(train_data["TBTL"], errors="coerce")
...
# Đánh giá trên tập training
    ...
    if len(test_data) > 0:
        ...
        # Xuất ra file CSV
        output_file = 'predicted_scores_TBTL.csv'
