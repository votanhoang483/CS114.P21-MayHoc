...
try:
    ...
    df_gt = pd.read_csv("./data/th-public.csv")
...
try:
    ...
    if 'TH' not in df_gt.columns:
        print("❌ Không tìm thấy cột 'TH' trong file ground truth")
        print(f"Các cột có sẵn: {df_gt.columns.tolist()}")
        exit(1)
    ...
    feature_columns = [col for col in student_features.columns 
                      if col not in ["username", "hash", "TH", "first_submission", "last_submission"]]
    
    X_train = train_data[feature_columns]
    y_train = pd.to_numeric(train_data["TH"], errors="coerce")
...
# Đánh giá trên tập training
    ...
    if len(test_data) > 0:
        ...
        # Xuất ra file CSV
        output_file = 'predicted_scores_TH.csv'
